    ...
    <div className="flex flex-col gap-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {!isMember && (
          <Link
            to="/projects"
            className="self-start md:self-center px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary-600 to-indigo-600 hover:from-primary-500 hover:to-indigo-500 text-xs font-semibold text-white shadow-lg shadow-primary-600/15 flex items-center gap-2 transition-all duration-300 cursor-pointer"
          >
            <FiPlus className="w-4 h-4" />
            Manage Workspaces
          </Link>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="glass-panel rounded-2xl p-5 border border-white/5 flex items-center justify-between shadow-lg">
          <div className="flex flex-col gap-1">
            <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">
              {isMember ? 'My Projects' : 'Total Projects'}
            </span>
          ...
          {isMember ? (
            <>
              <div className="flex flex-col gap-1">
                ...
                <span className="text-2xl font-extrabold text-white">{stats.activeProjects}</span>
              </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-white/5 flex items-center justify-between shadow-lg">
          <div className="flex flex-col gap-1">
            <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">
              {isMember ? 'My Tasks Assigned' : 'Total Tasks'}
            </span>
            <span className="text-2xl font-extrabold text-white">{stats.totalTasks}</span>
          </div>
        ...
        <div className="glass-panel rounded-2xl p-5 border border-white/5 flex items-center justify-between shadow-lg">
          <div className="flex flex-col gap-1">
            <span className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">
              {isMember ? 'Pending Tasks' : 'Pending Backlog'}
            </span>
            <span className="text-2xl font-extrabold text-white">{stats.pendingTasks}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel rounded-2xl p-6 border border-white/5 shadow-lg flex flex-col justify-between">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center gap-2">
            ...
            {isMember ? 'My Tasks Gauge' : 'Workspace Completion Gauge'}
          </h3>
          <div className="flex flex-col items-center justify-center py-6 gap-5">
            ...
            <p className="text-center text-slate-400 text-xs font-light max-w-[200px] leading-relaxed">
              {isMember
        ...
        <div className="glass-panel rounded-2xl p-6 border border-white/5 shadow-lg flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-5 py-2">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-400 font-medium">{stats.priorityHigh} tasks</span>
              </div>
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-400 font-medium">{stats.priorityMedium} tasks</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-slate-400 font-medium">{stats.priorityLow} tasks</span>
              </div>
              <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                <div
          ...
          <div className="border-t border-slate-800/80 pt-3 mt-4 text-[10px] text-slate-500 text-center font-light uppercase tracking-wider">
            {isMember ? 'Your Assigned Backlog:' : 'Total Active Backlog:'} {stats.totalTasks} Tasks
          </div>
        </div>
        ...
        <div className="glass-panel rounded-2xl p-6 border border-white/5 shadow-lg flex flex-col">
          <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4">
            {isMember ? 'My Assigned Tasks Activity' : 'Recent Backlog Activity'}
          </h3>
          <div className="flex-grow flex flex-col gap-4 overflow-y-auto max-h-[220px] pr-1">
            {recentTasks.length === 0 ? (
              <div className="h-full flex items-center justify-center text-slate-500 text-xs font-light py-10">
              ...
              recentTasks.map((t) => (
                <div key={t._id} className="flex gap-3 text-xs leading-normal">
                  <div className="flex-shrink-0 mt-0.5">
                    {t.status === 'done' ? (
                      <span className="w-5 h-5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold">
                        ✓
                      </span>
                    ) : (
                      <span className="w-5 h-5 rounded-lg bg-primary-500/10 border border-primary-500/20 text-primary-400 flex items-center justify-center font-bold">
                        •
                      </span>
                    )}
                  </div>
          ...
          {recentProjects.map((p) => (
            ...
            >
              ...
              <div className="flex items-center justify-between border-t border-slate-800/60 pt-3 text-[10px] text-slate-500">
                <span>
                  Members: <strong className="text-slate-400">{p.members?.length || 1}</strong>
                </span>
                <span className="flex items-center gap-1 group-hover:text-primary-400 transition-colors">
                  Open Board <FiArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition-transform" />
                </span>
              </div>
            </div>
          ))}
